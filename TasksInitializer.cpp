...
void TasksInitializer::registerTaskTypes(){
  //WATER PLANTS
    ...
    std::list<Parameter> parameters1;
  ...
  //LED TEST
    ...
    std::list<Parameter> parameters1LedTest;
  ...
  TaskType taskTypeWaterPumpSwitch(static_cast<int>(TASK_WATER_PUMP_SWITCH), "Water pump switch", "Turn on and off the water pump", parameterWaterPumpSwitch.toParameter());
    ...
    StatefulParameterSlider parameterLedSlider("LED brightness", "Control the LED brightness.", 1.0, 0, 100);
    //StatefulParameterSlider parameterLedSlider("LED brightness", "Control the LED brightness.");
   // StatefulParameterSlider parameterLedSlider();
  TaskType taskTypeLedSlider(static_cast<int>(TASK_LED_SLIDER), "LED slider", "Control the LED brightness.", parameterLedSlider.toParameter());
  ...
  TaskController::instance().addTaskType(taskTypeLedSlider);
...
void TasksInitializer::registerTasks(){
  ...
  TaskController::instance().addTask(new TaskLedSlider());
}